#!/bin/bash
#
# Script Name: show_services.sh
# Description: Displays running and stopped services in colorful tables along with counts.
# Highlights web servers and databases with special colors.
# Compatible with: CentOS 7, AlmaLinux, Ubuntu, Debian
# Author: Eisa Mohammadzadeh

# Color definitions
RED='\033[1;31m'
GREEN='\033[1;32m'
CYAN='\033[1;36m'
YELLOW='\033[1;33m'
NC='\033[0m'

# Check systemctl
if ! command -v systemctl &> /dev/null; then
    echo -e "${RED}Error: systemctl not found. Your system may not use systemd.${NC}"
    exit 1
fi

# Web and DB service patterns
web_patterns="httpd|nginx|apache2|lsws|litespeed"
db_patterns="mysql|mariadb|mysqld"

# Get services
active_services=$(systemctl list-units --type=service --state=running --no-pager --no-legend | awk '{print $1}')
inactive_services=$(systemctl list-units --type=service --state=inactive --no-pager --no-legend | awk '{print $1}')

# Convert to arrays
IFS=$'\n' read -rd '' -a active_array <<<"$active_services"
IFS=$'\n' read -rd '' -a inactive_array <<<"$inactive_services"

# Function to detect service type and return color
get_service_color() {
    local name="$1"
    if [[ "$name" =~ $web_patterns ]]; then
        echo -e "$CYAN"
    elif [[ "$name" =~ $db_patterns ]]; then
        echo -e "$YELLOW"
    else
        echo -e ""
    fi
}

# Function to print colored table with count
print_table() {
    local title="$1"
    local default_color="$2"
    local services=("${!3}")
    local count=${#services[@]}

    echo -e "${default_color}==================== ${title} ====================${NC}"
    printf "${default_color}%-40s${NC}\n" "Service Name"
    echo -e "${default_color}----------------------------------------${NC}"
    for svc in "${services[@]}"; do
        color=$(get_service_color "$svc")
        if [ -n "$color" ]; then
            printf "${color}%-40s${NC}\n" "$svc"
        else
            printf "%-40s\n" "$svc"
        fi
    done
    echo -e "${default_color}Total: $count service(s)${NC}"
    echo
}

# Print both tables
print_table "Running Services" "$GREEN" active_array[@]
print_table "Stopped Services" "$RED" inactive_array[@]
